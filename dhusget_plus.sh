...
while getopts ":d:u:p:l:P:q:C:m:i:t:s:e:S:E:f:c:T:o:V:h:F:R:D:r:O:N:L:n:w:W:Q:" opt; do
 case $opt in
	d)
...
if [ -z $TIME ] && [ -z $TIMEFILE ] && [ -z $ROW ] && [ -z $PRODUCT_TYPE ] && [ -z $ORBIT ] && [ -z $FREE_SUBQUERY_CHECK ] && [ -z $SENSING_TIME ] && [ -z $MISSION ] && [ -z $INSTRUMENT ];
then
     echo ""
...
if [ ! -z $ORBIT ];then
	...
	export QUERY_STATEMENT="$QUERY_STATEMENT relativeorbitnumber:$ORBIT"
	QUERY_STATEMENT_CHECK='OK'
fi
if [ ! -z $TIME ];then
	if [ ! -z $QUERY_STATEMENT_CHECK ]; then
